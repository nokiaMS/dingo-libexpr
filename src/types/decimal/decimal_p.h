...
class DecimalP {
 public:
  ...
  DecimalP(const long var) : m_ptr(std::make_shared<Decimal>(Decimal(var))) {
  }

  DecimalP(const double var) : m_ptr(std::make_shared<Decimal>(Decimal(var))) {
  }

  DecimalP(const std::string& str) : m_ptr(std::make_shared<Decimal>(Decimal(str))) {
  }
